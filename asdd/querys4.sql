...
begin
	...
	loop
		...
		if red.puntaje1<red.puntaje2 then
			...
			isitnull:=(select id_equipo from posiciones_dadafecha pp 
					where pp.id_equipo=red.id_equipo2 limit 1);
			if isitnull is null then
				insert into posiciones_dadafecha (id_equipo , puntaje) values (red.id_equipo2, 2);
			...
			isitnull:=(select id_equipo from posiciones_dadafecha pp 
					where pp.id_equipo=red.id_equipo1 limit 1);
			
			--para que agregue tambien al contrincante que perdio pero sin puntaje \
			if isitnull is null then
				insert into posiciones_dadafecha (id_equipo , puntaje) values (red.id_equipo1, 0);
		...
		--si es que el puntaje del primer equipo es peor que del segundo, lo guarda en la tabla temporal
			...
			if isitnull is null then
				insert into posiciones_dadafecha (id_equipo , puntaje) values (red.id_equipo1, 2);
			else 
				update posiciones_dadafecha set puntaje=puntaje+2 
	...
	create or replace view query4 as 
		(select e.nombre_equipo as Equipo, pd.puntaje 
		from posiciones_dadafecha pd 
		join equipos e on e.id_equipo=pd.id_equipo
		order by pd.puntaje DESC);
end;
 $$ language plpgsql;
